   ...
   for (;;)
   {
      ...
      if (FD_ISSET(csp->cfd, &rfds))
      {
         ...
         len = read_socket(csp->cfd, buf, max_bytes_to_read);
          log_error(LOG_LEVEL_INFO,"read from client %s",buf);
         if (len <= 0)
         {
...
static void serve(struct client_state *csp)
{
   ...
   csp->flags &= ~CSP_FLAG_ACTIVE;
    ...
    log_error(LOG_LEVEL_ERROR, "VPN log thread end");
    pthread_exit(0);
...
int shadowpath_closeHttp(int fd){
    close_socket(fd);
    g_terminate = 1;
    return 0;
}
...
#define SELECTED_ONE_OPTION
         {
            ...
            errno = pthread_create(&the_thread, &attrs,
               (void * (*)(void *))serve, csp);
             printf("VPN log pthread created %d",++threadCount);
            child_id = errno ? -1 : 0;
